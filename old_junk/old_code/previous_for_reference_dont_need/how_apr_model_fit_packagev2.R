...
### Fit Distributions
#################################################
### USE PALEO OBJECTS AND NORMALIZE FROM PALEO_NEWTRY

annual_obs_norm <- fit_norm_dist(flow_series=annual_obs_ts, distr="logis", ref_period=ref_period)
annual_rec_norm <- fit_norm_dist(flow_series=annual_rec_ts, distr="logis", ref_period=ref_period)
monthly_obs_norm <- fit_norm_dist(flow_series=monthly_obs_ts, distr=rep("gamma",12), ref_period=ref_period)
...
ap_model_rec <- flow_reconstr(recon_model=ap_model_fit)
...
lag_fit <- fit_model(method="apr", regmethod="lm", reconst_data=annual_obs_ts, annual_norm=annual_obs_norm, monthly_norm=monthly_obs_norm, pred_ts=list(clim_ind_ts, pc_ts), reg_eq = reg_eq, monthly_obs=monthly_obs_ts)
### Run Reconstruction
lag_fit_recon <- flow_reconstr(recon_model=lag_fit, post_proc=FALSE)
lag_fit_recon_postproc <- flow_reconstr(recon_model=lag_fit, post_proc=TRUE)
...
### Fit model to observed annual and save coefficients
lag_fit <- fit_model(method="apr", regmethod="lm", reconst_data=annual_rec_ts, annual_norm=annual_rec_norm, monthly_norm=monthly_obs_norm, pred_ts=list(clim_ind_ts, pc_ts), reg_eq = reg_eq, monthly_obs=monthly_obs_ts)
### Run Reconstruction
lag_fit_recon <- flow_reconstr(recon_model=lag_fit, post_proc=FALSE)
lag_fit_recon_postproc <- flow_reconstr(recon_model=lag_fit, post_proc=TRUE)
