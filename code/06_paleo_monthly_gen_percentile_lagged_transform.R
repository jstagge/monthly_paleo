...
output_name <- "percentile_lag_model"
...
model_path <- file.path(file.path(output_path, "paleo_monthly_gen"), "percentile_lag_model")
norm_model <- read.csv(file.path(model_path, paste0(site_id,"_percentile_lag_model_coef_",data_name,".csv")),  row.names = 1)

### Run Percentile Model
month_ts <- perc_model_lag(annual_rec=annual_data, monthly_param=monthly_param, annual_param=annual_param, norm_model=norm_model, first_month_wy=first_month_wy, data_name=data_name)

### Save Results
save_reconstruction(month_ts, site_id, site_name, output_name, data_name="observ_annual", method="Percentile Lag Model using True Annual Mean Flow")
...
if (site_id == "10109001") {
	...
	### Run Percentile Model
	month_ts <- perc_model_lag(annual_rec=annual_data, monthly_param=monthly_param, annual_param=annual_param, norm_model=norm_model, first_month_wy=first_month_wy, data_name="annual_flow_rec_local_m3s")

	### Save Results
	save_reconstruction(month_ts, site_id, site_name, output_name, data_name="rec_local", method="Percentile Lag Model using Local Reconstructed MAF")
	...
	### Run Percentile Model
	month_ts <- perc_model_lag(annual_rec=annual_data, monthly_param=monthly_param, annual_param=annual_param, norm_model=norm_model, first_month_wy=first_month_wy, data_name="annual_flow_rec_region_m3s")

	### Save Results
	save_reconstruction(month_ts, site_id, site_name, output_name, data_name="rec_region", method="Percentile Lag Model using Regional Reconstructed MAF")
	...
	### Run Percentile Model
	month_ts <- perc_model_lag(annual_rec=annual_data, monthly_param=monthly_param, annual_param=annual_param, norm_model=norm_model, first_month_wy=first_month_wy, data_name="annual_flow_rec_m3s")

	### Save Results
	save_reconstruction(month_ts, site_id, site_name, output_name, data_name="rec", method="Percentile Lag Model using Reconstructed MAF")
